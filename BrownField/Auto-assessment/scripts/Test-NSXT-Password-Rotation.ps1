...
function Test-NSXT-Password-Rotation {
    param (
        [SecureString]$token,
        [PSCustomObject]$sddc
    )
    try {
        ...
        if ($response -and $response.value -and $response.value.Count -gt 0) {
            ...
            $matchingItem = $response.value | Where-Object { $_.operationName.value -eq "Microsoft.AVS/privateClouds/rotateNsxtPassword/action" } | Select-Object -First 1
        
            # Check if a matching item was found
            if (-not $matchingItem) {
                $Global:recommendations += Get-Recommendation -type "NSXTPasswordRotation" `
                                            -sddcName $sddcDetails.sddcName
            }                    
        }
    }
    catch {
        Write-Error "NSX-T Password Rotation Test failed: $_"
        return
    }
}